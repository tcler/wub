...
namespace eval ::Scgi {
    ...
    proc read_length {sock data args} {
        ...
        if {$colonIdx == -1} {
            ...
            fileevent $sock readable [namespace code [list read_length $sock $data {*}$args]]
        } else {
            set length [string range $data 0 $colonIdx-1]
            set data [string range $data $colonIdx+1 end]
            read_headers $sock $length $data {*}$args
        }
    }
	...
	Debug.sscgi {SCGI $sock: $headers ($args)}
