...
namespace eval Query {
	...
	set str [string map [list + { } "\\" "0x81"] $str]

	# prepare to process all %-escapes
	regsub -all -- {%([A-Fa-f0-9][A-Fa-f0-9])} $str {\\u00\1} str
	
	# process \u unicode mapped chars
	return [string map [list "0x81" "\\"] [subst -novar -nocommand $str]]
    }
    ...
    proc encode {string} {
	variable map
	# map odd characters away - note: % must be first
	return [string map [list % $map(%) {*}[array get map]] $string]
    }
