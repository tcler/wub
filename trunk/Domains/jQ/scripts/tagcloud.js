...
*/
function TagCloud(cntr,sstyle,urlstub, carray,chighlight)
{
	...
	function drawCircle(nd)
	{
		...
		addToGridPos(x,y,c)
		drawn.push(c)
		renderCircle(c,nd.name, nd.r, nd.g, nd.b)
	...
	function renderCircle(c,name,r,g,b)
	{	
		var cvs = null
		if(isIE)
		{
			...
			//cvs.innerHTML = '0'
			cvs.innerHTML = "<v:oval style='width:"+(c.r*2)+"px; height:"+(c.r*2)+"px; z-index: 1' fillcolor='rgb("+r+","+g+","+b+")' stroked='false'></v:oval>";
	...
	function showAbout(name,evt)
	{
		...
		if(!tl)
		{
			...
			tl.style.color= '#ff358b'
			tl.style.fontWeight= 'bold'
			tl.style.fontSize='24px'
			tl.style.fontFamily='"Î¢ÈíÑÅºÚ",simsun,"ËÎÌå"'
